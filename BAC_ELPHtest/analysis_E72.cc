void analysis_E72(){
  ...
  for(int i=0;i<M;i++){
    for(int j=0;j<X;j++){
      eff[i][j] = new TEfficiency(Form("eff%d%d",i,j),Form("p_{#pi} = %d MeV/c;Y [mm];Efficiency",mom[i]),110,-60,50);
  ...
  for(int i=0;i<X;i++){
	...
	for(int n=0;n<5000;n++){
	  ...
	  for(int u=0;u<nhMppc[i][j][k];u++){
	    if(mppcnum[i][j][k][u]==2||mppcnum[i][j][k][u]==3)evt[i][j][k]++;
	  }
	  //hist_simul[i][j][k]->Fill(nhMppc[i][j][k]);
	  hist_simul[i][j][k]->Fill(evt[i][j][k]);
	  //if(nhMppc[i][j][k]>12)passed =1;
	  if(evt[i][j][k]>6)passed =1;
	  else{passed = 0;}
